...
expName = 'SYM_CABI_RSfMRI_1'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='./data/SYM_CABI_RSfMRI_1_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    while continueRoutine and routineTimer.getTime() < 25.0:
        ...
        if rs_cross.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
            ...
            rs_cross.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(rs_cross, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'rs_cross.started')
            # update status
        ...
        if rs_cross.status == STARTED:
            ...
            if tThisFlipGlobal > rs_cross.tStartRefresh + 500 - frameTolerance:
                ...
                rs_cross.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rs_cross.stopped')
                # update status
...
if __name__ == '__main__':
    ...
    valueType0 = [0]; valueType1 =  [1]; valueType2 = [2]; valueType3 = [3];
    
    expName = 'SYM_CABI_RSfMRI_1';
    ...
    time.sleep(2)
    rcs.sendAnnotation("t1", "stimulus")
    time.sleep(2)
    rcs.sendAnnotation("t2", "stimulus")
    time.sleep(2)
    rcs.sendAnnotation("t3", "stimulus")
    ...
    for iter in range(20) :
        time.sleep(2)
        port.write(valueType0);outlet.push_sample(valueType0);
        time.sleep(1)
        port.write(valueType1);outlet.push_sample(valueType1);
        time.sleep(1)
        port.write(valueType2);outlet.push_sample(valueType2);
        time.sleep(3)
        port.write(valueType3);outlet.push_sample(valueType3);
